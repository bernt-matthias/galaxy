...
class DependencyResolversView:
    ...
    def show_dependencies(self, tool_requirements_d, installed_tool_dependencies=None, **kwd):
        ...
        dependencies_per_tool = {
            tool: self._dependency_manager.requirements_to_dependencies(requirements, **to_deps_kwds)
    ...
    def _install_dependency(self, index, **payload):
        ...
        if resolver.read_only:
            raise exceptions.RequestParameterInvalidException(
                "Attempted to install on a read_only dependency resolver."
            )
    ...
    def installable_resolvers(self):
        ...
        return [
            index
            for index, resolver in enumerate(self._dependency_resolvers)
            if not resolver.read_only and not resolver.disabled
        ]
    ...
    def summarize_tools(self, **kwds):
        ...
        for tid, tool in self._app.toolbox.tools_by_id.items():
            ...
            status = self.get_requirements_status(
                tool_requirements_d={tid: requirements},
...
class ContainerResolutionView:
    ...
    def resolve(self, index: Optional[str] = None, **kwds) -> Dict[str, Any]:
        ...
        if resolved_container_description:
            status = ContainerDependency(
                resolved_container_description.container_description,
                container_resolver=resolved_container_description.container_resolver,
            ).to_dict()
        else:
    ...
    def _container_resolver(self, index: str):
        return self._container_resolvers[int(index)]
