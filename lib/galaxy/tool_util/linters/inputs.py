    ...
    def lint(cls, tool_source: "ToolSource", lint_ctx: "LintContext"):
        ...
        for param in inputs:
            ...
            if template or action or label:
                ...
                if template:
                    lint_ctx.warn(
                        f"Param input [{param_name}] {only}used in a template macro, use a macro token instead.", linter=cls.name(), node=param
                    )
                if action:
                    lint_ctx.warn(
                ...
                if label:
                    lint_ctx.warn(
                        f"Param input [{param_name}] {only}found in a label attribute, this is discouraged.", linter=cls.name(), node=param
                    )
                continue

            if change_format:
                lint_ctx.error(f"Param input [{param_name}] is only used in a change_format tag", linter=cls.name(), node=param)
            else:
                lint_ctx.error(f"Param input [{param_name}] not found in command or configfiles.", linter=cls.name(), node=param)
