...
TEST_IDS = [
    ...
    'inputs: parameter in command',
    'inputs: parameter in command with inputs configfile',
...
>>>>>>> b323dac30d (lint for unused inputs)
...
def test_inputs_parameter_in_command(lint_ctx):
    tool_source = get_xml_tool_source(PARAMETER_IN_COMMAND)
    run_lint(lint_ctx, inputs.lint_inputs, tool_source)
...
def test_inputs_parameter_in_command_with_inputs(lint_ctx):
    tool_source = get_xml_tool_source(PARAMETER_IN_COMMAND_WITH_INPUTS)
    run_lint(lint_ctx, inputs.lint_inputs, tool_source)
