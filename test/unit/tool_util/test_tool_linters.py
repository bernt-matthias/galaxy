...
TESTS = [
    ...
    (
        ...
        lambda x:
            ...
            and 'Param input [2] is not a valid Cheetah placeholder.' in x.error_messages
            ...
            and len(x.info_messages) == 1 and len(x.valid_messages) == 0 and len(x.warn_messages) == 1 and len(x.error_messages) == 3
    ),
    (
    ...
    (
        INPUTS_USED_PARAMETER_IN_COMMAND, inputs.lint_inputs_used,
        lambda x:
            "Param input [hey_a_missing_parameter] not found in command or configfiles." in x.error_messages
            and 'Param input [change_format_parameter] is only used in a change_format tag' in x.error_messages
            and len(x.info_messages) == 0 and len(x.valid_messages) == 0 and len(x.warn_messages) == 0 and len(x.error_messages) == 2
    ),
    (
        INPUTS_USED_OTHER, inputs.lint_inputs_used,
        lambda x:
            'Param input [variable_in_template_token] only used in a template macro, use a macro token instead.' in x.warn_messages
            and 'Param input [action_option_reference] only found in output actions.' in x.warn_messages
            and 'Param input [action_filter_reference] only found in output actions.' in x.warn_messages
            and 'Param input [action_conditional_reference] only found in output actions.' in x.warn_messages
            and "Param input [param_that_is_used_only_in_output_label] only found in a label attribute." in x.warn_messages
            and len(x.info_messages) == 0 and len(x.valid_messages) == 0 and len(x.warn_messages) == 5 and len(x.error_messages) == 0
    ),
    (
        INPUTS_USED_PARAMETER_IN_COMMAND_WITH_INPUTS, inputs.lint_inputs_used,
        lambda x:
            "Param input [param_that_is_not_in_inputs_configfile] not found in command or configfiles. Does the present inputs config miss the 'data_style' attribute?" in x.error_messages
            and len(x.info_messages) == 0 and len(x.valid_messages) == 0 and len(x.warn_messages) == 0 and len(x.error_messages) == 1
    ),
    (
        INPUTS_USED_PARAMETER_IN_COMMAND_WITH_INPUTS2, inputs.lint_inputs_used,
        lambda x:
            'Param input [param_that_is_not_in_inputs_configfile] only used inputs configfile inputs, but this is not used in the command' in x.error_messages
            and len(x.info_messages) == 0 and len(x.valid_messages) == 0 and len(x.warn_messages) == 0 and len(x.error_messages) == 1
    ),
    (
...
TEST_IDS = [
    ...
    'inputs used: parameter in command',
    'inputs used: parameter in command with inputs configfile',
    'inputs used: parameter in command unused inputs configfile',
    'inputs used: other warnings',
...
>>>>>>> df0fe76b68 (linting of unused parameters)
...
def test_inputs_used_parameter_in_command(lint_ctx):
    ...
    run_lint(lint_ctx, inputs.lint_inputs, tool_source)
    assert "Param input [cond_select] only used in the select of a conditional, which should be OK." in x.info_messages
    assert "Param input [hey_a_missing_parameter] not found in command or configfiles." in x.error_messages
    assert len(x.info_messages) == 2
    assert not x.warn_messages
    assert len(x.error_messages) == 1


def test_inputs_used_other(lint_ctx):
    tool_source = get_xml_tool_source(INPUTS_USED_OTHER)
    run_lint(lint_ctx, inputs.lint_inputs, tool_source)
    assert 'Param input [variable_in_template_token] only used in a template macro, use a macro token instead.' in x.warn_messages
    assert 'Param input [action_option_reference] only found in output actions.' in x.warn_messages
    assert 'Param input [action_filter_reference] only found in output actions.' in x.warn_messages
    assert 'Param input [action_conditional_reference] only found in output actions.' in x.warn_messages
    assert "Param input [param_that_is_used_only_in_output_label] only found in a label attribute." in x.warn_messages
    assert not x.info_messages
    assert not x.valid_messages
    assert len(x.warn_messages) == 5
    assert not x.error_messages
...
def test_inputs_used_parameter_in_command_with_inputs(lint_ctx):
    ...
    run_lint(lint_ctx, inputs.lint_inputs, tool_source)
    assert "Param input [param_that_may_be_in_inputs_configfile] may be unused. Double check that is used in the inputs configfile [inputs] and that this file is used properly." in x.info_messages
    assert "Param input [param_that_is_not_in_inputs_configfile] not found in command or configfiles. Does the present inputs config miss the 'data_style' attribute?" in x.error_messages
    assert len(x.info_messages) == 2
    assert not x.warn_messages
    assert len(x.error_messages) == 1
