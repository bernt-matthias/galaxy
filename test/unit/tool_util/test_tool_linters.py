...
INPUTS_PARAM_NAME = """
<tool>
    <command>$2 $valid $param_name</command>
    <inputs>
        <param type="text"/>
...
INPUTS_CONDITIONAL = """
<tool>
    <command>$select $select3 $select4 $bool $text $optionalselect $multipleselect $select3 $label_select $missing_option</command>
    <inputs>
        <conditional>
...
<tool>
    <command>
        $select_noopt $select_noopts $select_fd_op $select_fd_fdt $select_noval_notext $select_meta_file_key_incomp
    </command>
    <inputs>
...
<tool>
    ...
    <command>
        ...
        $sec.paired_collection2['reverse']
        $getVar("hard")
        ## skipping $cond_select, note that this comment is ignored as well
...
INPUTS_PARAMETER_IN_COMMAND_WITH_INPUTS = """
<tool>
    <command>
...
def test_inputs_parameter_in_command(lint_ctx):
    tool_source = get_xml_tool_source(INPUTS_PARAMETER_IN_COMMAND)
    run_lint(lint_ctx, inputs.lint_inputs, tool_source)
...
def test_inputs_parameter_in_command_with_inputs(lint_ctx):
    tool_source = get_xml_tool_source(INPUTS_PARAMETER_IN_COMMAND_WITH_INPUTS)
    run_lint(lint_ctx, inputs.lint_inputs, tool_source)
